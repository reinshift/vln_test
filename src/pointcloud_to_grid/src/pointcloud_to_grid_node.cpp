...
static int g_occupied_value = 100;
static int g_free_value = 0;
static int g_unknown_value = -1;
static bool g_require_transform = true;
...
void paramsCallback(my_dyn_rec::MyParamsConfig &config, uint32_t level)
{
  ...
  if (nh_ptr) {
    nh_ptr->param("min_z", g_min_z, 0.05);
    nh_ptr->param("max_z", g_max_z, 1.5);
    nh_ptr->param("occupied_value", g_occupied_value, 100);
    nh_ptr->param("free_value", g_free_value, 0);
    nh_ptr->param("unknown_value", g_unknown_value, -1);
    nh_ptr->param("require_transform", g_require_transform, true);
  }
  ...
  if (nh_ptr) {
    ...
    if (last_igrid_topic != grid_map.mapi_topic_name) {
      ...
      ROS_INFO("Advertising new occupancy grid topic: %s", last_igrid_topic.c_str());
    }
  ...
  std::fill(intensity_grid->data.begin(), intensity_grid->data.end(), static_cast<signed char>(g_free_value));
  std::fill(height_grid->data.begin(), height_grid->data.end(), static_cast<signed char>(g_unknown_value));
...
void pointcloudCallback(const sensor_msgs::PointCloud2::ConstPtr &msg)
{
  ...
  if (intensity_grid->data.size() != expected || height_grid->data.size() != expected) {
    try {
      intensity_grid->data.assign(expected, static_cast<signed char>(g_free_value));
      height_grid->data.assign(expected, static_cast<signed char>(g_unknown_value));
    } catch (...) {
  ...
  if (!src_frame.empty() && src_frame != target_frame && g_tf_buffer) {
    try {
      ...
      if (!tf_st.header.frame_id.empty()) {
        ...
        ROS_WARN_THROTTLE(5.0, "No TF from %s to %s", src_frame.c_str(), target_frame.c_str());
        if (g_require_transform) {
          return;
        }
      }
    } catch (const std::exception &e) {
      ROS_WARN_THROTTLE(5.0, "TF transform failed: %s", e.what());
      if (g_require_transform) {
        return;
      }
    }
  ...
  try {
    ...
    for (; iter_x != iter_x.end(); ++iter_x, ++iter_y, ++iter_z) {
      ...
      intensity_grid->data[index] = static_cast<signed char>(std::max(-128, std::min(127, g_occupied_value)));
      const signed char height_char = static_cast<signed char>(std::max(-128.0, std::min(127.0, height_val)));
      if (height_grid->data[index] < height_char) {
        height_grid->data[index] = height_char;
      }
    }
  } catch (const std::exception &e) {
